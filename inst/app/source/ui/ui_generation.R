...
tabItem("generation",
        ...
        fluidRow(
          ...
          box(title = 'Step 1: Input data of 3D analysis',
              width=6,status = 'primary', solidHeader = T,
              h4('It may take a while for the apple to respond for big dataset. Please wait until one process done before go to next step.'),
              column(width = 12,
                     # wellPanel(
                     # style = "background-color: #ffffff;",
              ...
              column(width = 12,
                     radioButtons(inputId = 'has_srep',label = '>Does the data have sequencing (technical) replicates?',
                       ...
                       helper(icon = "question-circle",
                              colour = NULL,
                              type = 'markdown',
                              title = 'Select factors of samples',
                              size = 'l',
